...
@pytest.fixture()
def test_vote(test_posts: list[Post], session: Session, test_user: User) -> None:
...
def test_votes_on_post(authorized_client: TestClient, test_posts: list[Post]) -> None:
...
def test_vote_twice_post(authorized_client: TestClient, test_posts: list[Post]) -> None:
    res = authorized_client.post(
        "/api/v1/votes/", json={"post_id": test_posts[0].id, "dir": 1}
    )
    logging.debug(res.json())
...
@pytest.mark.usefixtures("test_vote")
def test_delete_vote(authorized_client: TestClient, test_posts: list[Post]) -> None:
    res = authorized_client.post(
...
def test_vote_post_non_exist(authorized_client: TestClient) -> None:
...
def test_vote_unauthorized_user(client: TestClient, test_posts: list[Post]) -> None:
